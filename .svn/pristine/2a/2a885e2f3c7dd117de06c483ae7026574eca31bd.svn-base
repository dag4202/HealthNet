...
def _show_users(request):
    ...
    if not "show_users" in s:
        if is_Doctor or is_Nurse or is_Admin:
            s["show_users"] = True 
        else:
            s["show_users"] = False
    return s["show_users"]
...
def settings(request):
    ...
    if request.method == "POST":
        s["show_users"] = False #(True if "show_users" in request.POST else False)
    return render_to_response("calendar/settings.html", add_csrf(request, show_users=s["show_users"]), context_instance=RequestContext(request))
...
#@login_required
#def main(request):
    #logger.info("accessed calendar year view")
...
#
        ...
        #lst.append((y, mlst))
#
    #return render_to_response("calendar/nMain.html", dict(years=lst, user=request.user, year=year,
                                                   #reminders=reminders(request)),
                                                   #context_instance=RequestContext(request))
@login_required
def main(request):
    #This function is a blatent copy-paste of month(), minus the input parameters.
    #I will re-write the url codestuffs after R1. -Alex
    ...
    return render_to_response("calendar/month.html", dict(year=year, month=month, user=request.user,
...
def day(request, year, month, day):
    ...
    if request.method == 'POST':
        ...
        if formset.is_valid():
            ...
            for entry in entries:
                ...
                entry.save()
                log = LogEntry(username=request.user.username, datetime=timezone.now(), description='appointment made/changed')
                log.save()
            return HttpResponseRedirect(reverse("calender:month", args=(year, month)))
    ...
    return render_to_response("calendar/day.html", add_csrf(request, entries=formset, year=year,
            month=month, mname=mnames[month-1], day=day, other_entries=other_entries, reminders=reminders(request)),
            context_instance=RequestContext(request))
